...
-- Create database user for CAS
-- 请根据实际环境修改密码
CREATE USER IF NOT EXISTS 'cas_user'@'%' IDENTIFIED BY 'cas_password';
GRANT ALL PRIVILEGES ON cas_db.* TO 'cas_user'@'%';
FLUSH PRIVILEGES;

-- CAS Service Registry table (auto-created by JPA, but can be customized here if needed)
-- CAS Ticket Registry table (auto-created by JPA)
